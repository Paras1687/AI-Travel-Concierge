...
//       {open && (
//         <div className="absolute top-full z-50 mt-2 left-1/2 -translate-x-1/2 sm:left-0 sm:translate-x-0 w-[min(92vw,320px)] bg-paper rounded-2xl shadow-soft border border-sand-200 p-4">
    ...
    <div ref={containerRef} className={`relative w-full ${className}`}>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          type="button"
          onClick={() => setOpen((o) => !o)}
          className="flex flex-col items-start gap-1 bg-[#FDFBF7] border border-stone-200 rounded-xl px-4 py-3 text-left hover:border-stone-400 transition-colors focus:outline-none focus:ring-2 focus:ring-[#c66542]"
        >
          <span className="text-[11px] font-bold uppercase tracking-wider text-stone-500">Start date</span>
        ...
        <button
          ...
          className="flex flex-col items-start gap-1 bg-[#FDFBF7] border border-stone-200 rounded-xl px-4 py-3 text-left hover:border-stone-400 transition-colors focus:outline-none focus:ring-2 focus:ring-[#c66542]"
        >
          <span className="text-[11px] font-bold uppercase tracking-wider text-stone-500">End date</span>
          <span className="flex items-center gap-2 text-sm font-medium text-stone-800">
            <CalendarIcon />
            {formatLong(endDate) || 'Select date'}
          </span>
      ...
      {open && (
        <div className="absolute top-[calc(100%+8px)] left-0 z-[999] w-[320px] bg-white rounded-2xl shadow-2xl border border-stone-100 p-5">
          ...
          <div className="flex items-center justify-between mb-4">
            <button
              ...
              onClick={() => changeMonth(-1)}
              disabled={!canGoPrev}
              className="p-1.5 rounded-full text-stone-500 hover:bg-stone-100 disabled:opacity-30 transition-colors"
            >
          ...
          <div className="grid grid-cols-7 mb-2">
            {WEEKDAY_LABELS.map((w, i) => (
              <div key={i} className="text-center text-[11px] font-bold text-stone-400">
                {w}
              </div>
            ))}
          </div>
            ...
            {cells.map((day, i) => {
              ...
              return (
                <div key={i} className="relative flex items-center justify-center h-10">
                  {inRange && <div className="absolute inset-y-0 left-0 right-0 bg-[#fef4f1]" />}
                  {isStart && (startDate && (endDate || hoverDate)) && (
                    <div className="absolute inset-y-0 left-1/2 right-0 bg-[#fef4f1]" />
                  )}
                  {isEnd && (
                    <div className="absolute inset-y-0 right-1/2 left-0 bg-[#fef4f1]" />
                  )}
                  <button
                    type="button"
                    disabled={disabled}
                    onClick={() => handleDayClick(day)}
                    onMouseEnter={() => setHoverDate(day)}
                    className={`relative w-9 h-9 rounded-full text-sm font-medium transition-all flex items-center justify-center z-10
                      ${disabled ? 'text-stone-300 cursor-not-allowed' : 'text-stone-700 hover:bg-stone-100 cursor-pointer'}
                      ${isEdge ? 'bg-[#c66542] text-white hover:bg-[#a65335] shadow-md' : ''}
                    `}
                  >
                    {day.getDate()}
                  </button>
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-stone-100">
            <p className="text-xs text-stone-500">
              {startDate && !endDate ? 'Select return date' : 'Tap a date to start'}
            </p>
            {(startDate || endDate) && (
              <button
                type="button"
                onClick={() => onChange({ startDate: null, endDate: null })}
                className="text-xs font-semibold text-[#c66542] hover:underline"
              >
                Clear
              </button>
            )}
