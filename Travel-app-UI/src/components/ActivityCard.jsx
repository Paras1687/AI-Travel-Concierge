  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 bg-paper rounded-xl shadow-card border border-sand-200 overflow-hidden hover:shadow-soft hover:-translate-y-0.5 transition-all">
      ...
      <div className="p-5 flex-1">
        ...
        <div className="flex items-center justify-between text-xs text-ink-500/80 mt-3 pt-2.5 border-t border-sand-200 flex-wrap gap-2">
          <span>⏱️ Duration: {activity.duration}</span>
          {activity.cost && (
            <span className="font-semibold text-forest bg-forest/10 px-2.5 py-0.5 rounded-md border border-forest/20">
              🎟️ Est. Cost: {activity.cost}
            </span>
          )}
        </div>
      </div>
